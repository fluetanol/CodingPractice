...
해설
처음 볼 때 상당한 난이도의 로직이니까 확실히 이해할 필요가 있다.
...
DP로 풀 생각이니 당연히 메모이제이션 할 공간복잡도가 늘어나는 것.)

방식으로 해둔다.


이후 2차 중복을 걸러낸다. 이는 크기 M에 따라 갈린다. 가령,
[1,2]라면 직관적으로 앞에 있는 0열은 의미 없음을 알 수 있다. (M=2일때, [A,0]성분의 영향력을 없애야 한다)
...
1 8 13 4 3 9
여기서 남아있는 4는 누적합이여서 1 3으로 해체가 가능하다
1 8 13 1 3 9
근데 이중 1열 출신 성분들을 뽑아보면
1 8
이 두놈이 생긴다.
1+8=9인데, 이는 곧 누적합 [1,0]위치에 있는 성분이다.(0열 성분)
...
에서 [2,2] = 68을 생각해본다면
유효한 범위는 13 9 16 11이 네가지이므로 0행과 0열의 둘다의 정보가 의미없음을 알 수 있다.
